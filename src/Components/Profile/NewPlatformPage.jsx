...
const NewPlatformPage = () => {
    ...
    const handleUpdate = async () => {
        try {
            const response1 = await axios.get(`https://codechef-api.vercel.app/handle/${usernames["Codechef"]}`);
            const response2 = await axios.get(`https://codeforces.com/api/user.info?handles=${usernames["Codeforces"]}`);
            const response3 = await axios.get(`https://leetcode-stats-api.herokuapp.com/${usernames["Leetcode"]}`);

            if (response1.status !== 200 || response2.status !== 200 || response3.status !== 200) {
                alert("Some platform username is not verified");
                return; 
            }
    ...
    const handleDelete = (platform) => {
        alert(`Delete clicked for ${platform}`);
        setUsernames((prevState) => ({
            ...prevState,
            [platform]: "",
        }));
